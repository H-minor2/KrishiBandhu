...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function loadDashboardData() {
      ...
      if (data.profile) {
        ...
        localStorage.setItem('krishi_bandhu_cached_profile', JSON.stringify(data.profile));
      }
      if (data.crops) {
        setCrops(data.crops);
        localStorage.setItem('krishi_bandhu_cached_crops', JSON.stringify(data.crops));
      }
  ...
  const handleAddCropEntry = async (e: React.FormEvent) => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('krishi_bandhu_cached_crops', JSON.stringify(optimisticCrops));
    }
    ...
    try {
      ...
      if (res.crop) {
        setCrops(currentCrops => {
          const syncedCrops = currentCrops.map(c => c.id === tempId ? { ...res.crop, sync_status: 'synced' as const } : c);
          if (typeof window !== 'undefined') {
            localStorage.setItem('krishi_bandhu_cached_crops', JSON.stringify(syncedCrops));
          }
          return syncedCrops;
        });
      }
      ...
      console.error("Offline or sync failed, crop remains pending in cache", error);
    }
  };
    ...
    <main className={`min-h-screen m-0 p-0 font-[Arial,Verdana,sans-serif] bg-slate-50 text-black ${fontSize}`}>
      ...
      <nav className="w-full bg-[#003366] text-white p-4 border-b border-black flex flex-wrap gap-4 justify-between items-center rounded-none shadow-none">
        <div className="flex items-center gap-4">
          ...
          <h1 className="m-0 text-xl font-bold tracking-tight">{t('dashboardTitle')}</h1>
        </div>
      ...
      <div className="w-full max-w-6xl mx-auto p-4 md:p-8 space-y-6">
        ...
        <div className="bg-white border-2 border-black p-6 rounded-none flex flex-wrap justify-between items-center gap-4">
          <div>
            ...
            <h2 className="text-2xl font-bold text-black mt-2 flex items-center gap-2">
              ...
              {profile?.full_name || (loading ? t('loadingProfile') : t('registeredFarmerLabel'))}
            </h2>
            <p className="text-sm text-gray-700 mt-1 flex items-center gap-1">
              <Phone className="w-4.5 h-4.5 mr-1.5"/> 
              <strong>{t('mobileLabel')}:</strong> {profile?.mobile_number || 'N/A'}  <strong className='flex items-center'> <Pin className='w-4.5 h-4.5 mx-1.5'/> {t('locationLabel')}:</strong> {profile?.location_address || (profile?.district ? `${profile.district}, ${profile.state}` : 'N/A')}
            </p>
          </div>
        ...
        <div className="w-full bg-[#FFD700] border-2 border-black p-4 flex flex-wrap justify-between items-center gap-4 cursor-pointer hover:bg-[#FACC15] transition-colors">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-black text-lg m-0 uppercase tracking-wide">Financial Distress Score</h3>
              <p className="text-sm font-medium text-black/80 m-0">Calculate risk assessment based on regional weather & market data</p>
            </div>
          </div>
          <Link 
            href="/distress-score"
        ...
        {loading ? (
          ...
          <div className="w-full border-2 border-dashed border-gray-400 p-10 text-center bg-white">
            ...
            <p className="text-sm text-gray-500 mt-1">{t('clickRegisterMsg')}</p>
            <button
              onClick={() => setIsModalOpen(true)}
            ...
            {crops.map((crop, idx) => (
              <div key={crop.id || idx} className="bg-white border-2 border-black p-6 rounded-xl space-y-3 relative">
                <div className="flex justify-between items-start border-b border-black pb-2">
                  <div>
                    <span className="text-xs font-bold text-[#003366] uppercase tracking-wider">{t('cropRecordNum')} #{idx + 1}</span>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    <span className="text-gray-500 font-medium">{t('landSizeLabel')}:</span>
                    <p className="font-bold">{crop.land_size} {crop.land_unit}</p>
                  </div>
                  <div>
                    <span className="text-gray-500 font-medium">{t('soilTypeDisp')}:</span>
                    <p className="font-bold">{crop.soil_type}</p>
                  </div>
                  <div>
                    <span className="text-gray-500 font-medium">{t('irrigationDisp')}:</span>
                    <p className="font-bold">{crop.irrigation_type}</p>
                  </div>
                  <div>
                    <span className="text-gray-500 font-medium">{t('sowingDateDisp')}:</span>
                    <p className="font-bold">{crop.sowing_date}</p>
                  </div>
                  <div>
                    <span className="text-gray-500 font-medium">{t('expectedHarvestDisp')}:</span>
                    <p className="font-bold">{crop.expected_harvest_date}</p>
                  </div>
                  <div>
                    <span className="text-gray-500 font-medium">{t('loanDueDateDisp')}:</span>
          ...
          <div className="bg-white border-2 border-black p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-xl">
            ...
            <form onSubmit={handleAddCropEntry} className="space-y-6">
              ...
              <div className="flex justify-end gap-3 mt-8 border-t border-black pt-4">
                ...
                <button
                  type="submit"
                  className="px-6 py-2.5 font-bold border border-black bg-[#003366] text-white hover:bg-blue-800 rounded-none cursor-pointer flex items-center gap-2"
                >
